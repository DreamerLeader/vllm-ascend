...
class ChunkedTokenDatabase:
    def __init__(self, metadata: KeyMetadata, block_size: int, partitions: list[int] | None):
        ...
        # Cross-TP support: stores producer's per-head block length
        self.producer_per_head_block_len: list[int] = []
        # Number of local KV heads for the current (consumer) config
        self.num_local_kv_heads: int = 1
        # Head dimension size (in elements)
        self.head_dim: int = 0
    ...
    def set_head_geometry(self, num_local_kv_heads: int, head_dim: int, element_size: int):
        """Set KV cache head geometry for cross-TP slicing.

        Args:
            num_local_kv_heads: Number of local KV heads on this rank.
            head_dim: Dimension per head.
            element_size: Bytes per element (e.g. 2 for fp16).
        """
        self.num_local_kv_heads = num_local_kv_heads
        self.head_dim = head_dim
        self.element_size = element_size
        # Per-head block length: block_size * 1 * head_dim * element_size
        per_head_bytes = self.block_size * head_dim * element_size
        self.producer_per_head_block_len = [per_head_bytes] * len(self.block_len)
        logger.info(
            "[ChunkedTokenDatabase] head_geometry: num_local_kv_heads=%d, "
            "head_dim=%d, element_size=%d, per_head_block_bytes=%d",
            num_local_kv_heads, head_dim, element_size, per_head_bytes,
        )

    def make_key_with_remote_head(self, chunk_hash: str, remote_head_or_tp_rank: int) -> "PoolKey":
        """Create a PoolKey using a remote (producer) head_or_tp_rank."""
        remote_metadata = KeyMetadata(
            model_name=self.metadata.model_name,
            head_or_tp_rank=remote_head_or_tp_rank,
            pcp_rank=self.metadata.pcp_rank,
            dcp_rank=self.metadata.dcp_rank,
            pp_rank=self.metadata.pp_rank,
        )
        return PoolKey(remote_metadata, chunk_hash)
    ...
    def prepare_value_per_head(
        ...
        block_ids: list[int],
        local_head_idx: int,
        num_local_heads_in_block: int,
    ) -> tuple[list[int], list[int]]:
        """Compute addresses for a single KV head within a block.

        The KV cache block layout is [block_size, num_local_kv_heads, head_dim].
        For a specific head h, the data at each token position t is at:
            offset = (t * num_local_kv_heads + h) * head_dim * element_size

        Since per-head data is NOT contiguous, this returns the base address
        and size for the ENTIRE block. The caller must use staging buffers
        to extract per-head data.

        Args:
            start: Start token index.
            end: End token index.
            block_ids: Block IDs allocated to this request.
            local_head_idx: Which local head to extract.
            num_local_heads_in_block: Total local heads in the source block.

        Returns:
            (addr_list, size_list) for the entire block data.
        """
