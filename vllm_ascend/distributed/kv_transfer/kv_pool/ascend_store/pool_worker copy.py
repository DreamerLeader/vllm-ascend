...
from vllm_ascend.distributed.kv_transfer.kv_pool.ascend_store.kv_transfer import (
    CrossTPKVCacheRecvingThread,
    CrossTPKVCacheSendingThread,
...
class KVPoolWorker:
    ...
    def __init__(
        ...
        # --- Cross-TP (GQA + TP asymmetry) configuration ---
        self.cross_tp_enabled = False
        self.tp_head_mapper: TPHeadMapper | None = None
        self.staging_buffers: dict[str, tuple[torch.Tensor, torch.Tensor]] | None = None
        self.prefill_tp_size = self.tp_size
        self.decode_tp_size = self.tp_size

        extra_config = vllm_config.kv_transfer_config.kv_connector_extra_config
        prefill_cfg = extra_config.get("prefill", {})
        decode_cfg = extra_config.get("decode", {})
        cfg_prefill_tp = prefill_cfg.get("tp_size", self.tp_size)
        cfg_decode_tp = decode_cfg.get("tp_size", self.tp_size)

        if cfg_prefill_tp != cfg_decode_tp and not self.use_mla:
            self.cross_tp_enabled = True
            self.prefill_tp_size = cfg_prefill_tp
            self.decode_tp_size = cfg_decode_tp
            self.tp_head_mapper = TPHeadMapper(
                total_num_kv_heads=self.num_kv_head,
                prefill_tp_size=cfg_prefill_tp,
                decode_tp_size=cfg_decode_tp,
            )
            logger.info(
                "[KVPoolWorker] Cross-TP ENABLED: prefill_tp=%d, decode_tp=%d, "
                "total_kv_heads=%d, role=%s, tp_rank=%d",
                cfg_prefill_tp,
                cfg_decode_tp,
                self.num_kv_head,
                self.kv_role,
            ...
            logger.info(
                "[KVPoolWorker] Cross-TP DISABLED (symmetric TP or MLA): "
                "tp_size=%d, num_kv_heads=%d, use_mla=%s",
                self.tp_size,
                self.num_kv_head,
                self.use_mla,
            )
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]):
        ...
        # --- Cross-TP: allocate and register staging buffers ---
        if self.cross_tp_enabled and self.tp_head_mapper is not None and not self.use_mla:
            self._allocate_staging_buffers(first_kv_cache, kv_caches, ptrs, lengths)

        self.m_store.register_buffer(ptrs, lengths)
        self.token_database.set_kv_caches_base_addr(self.kv_caches_base_addr)
        self.token_database.set_block_len(self.block_len)

        # Set head geometry for cross-TP address computation
        if not self.use_mla and not self.use_sparse and len(first_kv_cache.shape) >= 4:
            # shape: [num_blocks, block_size, num_local_kv_heads, head_dim]
            num_local_kv_heads = first_kv_cache.shape[2]
            head_dim = first_kv_cache.shape[3]
            elem_size = first_kv_cache.element_size()
            self.token_database.set_head_geometry(num_local_kv_heads, head_dim, elem_size)

        # --- Thread creation ---
        if self.cross_tp_enabled and self.tp_head_mapper is not None:
            self._create_cross_tp_threads()
    ...
    def _allocate_staging_buffers(
        ...
        """
        Allocate staging buffers for cross-TP head remapping.

        When the producer (Decode) has a different number of local KV heads
        per rank than the consumer (Prefill), we need staging buffers to:
        1. Receive the producer's block (with producer's head count).
        2. Slice/rearrange heads to match the consumer's layout.

        Staging buffer shape per layer: [block_size, producer_heads_per_rank, head_dim]
        for both K and V.
        """
        assert self.tp_head_mapper is not None

        if self.use_mla or self.use_sparse:
            logger.info("[KVPoolWorker] Skipping staging buffers for MLA/sparse models")
            return

        # Infer head_dim from KV cache shape: [num_blocks, block_size, num_local_heads, head_dim]
        if len(first_kv_cache.shape) < 4:
            logger.warning(
                "[KVPoolWorker] Cannot allocate staging buffers: "
                "unexpected KV cache shape %s",
                first_kv_cache.shape,
            )
            return
        ...
        # Determine the PRODUCER's heads per rank for staging size
        producer_heads = self.tp_head_mapper.decode_heads_per_rank

        self.staging_buffers = {}
        for layer_name in kv_caches:
            # One staging block per layer: [block_size, producer_heads, head_dim]
            staging_k = torch.zeros(
                (self.original_block_size, producer_heads, head_dim),
                dtype=dtype,
                device=device,
            )
            ...
            self.staging_buffers[layer_name] = (staging_k, staging_v)

            # Register staging buffers for RDMA
            k_ptr = staging_k.data_ptr()
            v_ptr = staging_v.data_ptr()
            k_len = staging_k.nelement() * staging_k.element_size()
            v_len = staging_v.nelement() * staging_v.element_size()
            ptrs.extend([k_ptr, v_ptr])
            lengths.extend([k_len, v_len])

        logger.info(
            "[KVPoolWorker] Allocated %d staging buffer pairs for cross-TP: "
            "shape=[%d, %d, %d], dtype=%s, device=%s, "
            "total_staging_bytes=%d",
            len(self.staging_buffers),
            ...
            device,
            sum(
                s_k.nelement() * s_k.element_size() + s_v.nelement() * s_v.element_size()
                for s_k, s_v in self.staging_buffers.values()
            ),
        )

    def _create_cross_tp_threads(self):
        """Create send/recv threads for cross-TP KV cache transfer."""
        assert self.tp_head_mapper is not None

        # Producer (Decode) side: save KV cache
        if self.kv_role in ["kv_producer", "kv_both"] or self.consumer_is_to_put:
            ready_event_sending = threading.Event()
            self.kv_send_thread = CrossTPKVCacheSendingThread(
                self.m_store,
                self.token_database,
                self.block_size,
                self.tp_rank,
            ...
            self.kv_send_thread.start()
            logger.info(
                "[KVPoolWorker] Started CrossTPKVCacheSendingThread: "
                "tp_rank=%d, role=%s",
                self.tp_rank,
                self.kv_role,
            )

        # Consumer (Prefill) side: load KV cache
        if self.kv_role in ["kv_consumer", "kv_both"] or self.load_async:
            ready_event = threading.Event()
            self.kv_recv_thread = CrossTPKVCacheRecvingThread(
                self.m_store,
                self.token_database,
                self.block_size,
                self.tp_rank,
            ...
            ready_event.wait()
            logger.info(
                "[KVPoolWorker] Started CrossTPKVCacheRecvingThread: "
                "tp_rank=%d, role=%s",
                self.tp_rank,
                self.kv_role,
            )
    ...
    def start_load_kv(self, metadata: AscendConnectorMetadata):
        ...
        for request in metadata.requests:
            ...
            # --- Cross-TP load path ---
            if self.cross_tp_enabled and self.kv_recv_thread is not None:
                logger.info(
                    "[KVPoolWorker] Cross-TP load: req=%s, token_len=%d, "
                    "prefill_tp=%d, decode_tp=%d, tp_rank=%d",
                    request.req_id,
                    token_len,
