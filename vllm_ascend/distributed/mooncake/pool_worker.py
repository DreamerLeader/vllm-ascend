...
class KVPoolWorker:
    ...
    def __init__(
        ...
        if self.num_kv_head < self.tp_size:
            ...
            self.local_head_indices = [self.head_or_tp_rank]
        else:
            ...
            self.local_head_indices = [
                start_head + i for i in range(heads_per_rank)
            ]
                ...
                if self.load_async:
                    ...
                    for start, end, key, head_idx in self.token_database.process_tokens(
                        ...
                        addr, size, _ = self.token_database.prepare_value(
                            start, end, request.block_ids, head_idx)
    ...
    def retrieve_layer(
        ...
        keys = []
        first_flag = True
        for start, end, key, _ in self.token_database.process_tokens(
                token_len, request.block_hashes, mask_num):
