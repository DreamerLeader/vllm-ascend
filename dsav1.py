...
_SHAREDKV_METADATA_MOCK_LOGGED = False


def _npu_sparse_attn_sharedkv_metadata_maybe_mock(*args, **kwargs):
    """Wraps torch.ops._C_ascend.npu_sparse_attn_sharedkv_metadata.

    When env DSA_MOCK_SHAREDKV_METADATA=1, skip the real op (suspected to
    hang) and return a placeholder tensor matching the op's documented
    output: shape [1024], dtype int32, on the same device as the first
    tensor input (matches torch_binding_meta.cpp:921).
    """
    import os
    global _SHAREDKV_METADATA_MOCK_LOGGED
    if os.environ.get("DSA_MOCK_SHAREDKV_METADATA", "0") != "1":
    ...
    for _key in (
        ...
        if isinstance(_t, torch.Tensor):
            ...
            break
    if device is None:
        _dev_str = kwargs.get("device")
        device = torch.device(_dev_str) if isinstance(_dev_str, str) else torch.device("cpu")

    if not _SHAREDKV_METADATA_MOCK_LOGGED:
        print(
            f"[DSA_MOCK] npu_sparse_attn_sharedkv_metadata MOCKED -> zeros(1024,int32) on {device}. "
            "kwargs(scalars only): "
            + ", ".join(
                f"{k}={v}"
        ...
        _SHAREDKV_METADATA_MOCK_LOGGED = True
...
class AscendDSAMetadataBuilder(AttentionMetadataBuilder[AscendDSAMetadata]):
    ...
    def build_prefill_metadata(
        ...
        if self.compressor_ratio <= 1:
            if self.prefill_ratio_to_sas_metadata.get(layer_name) is None:
                self.prefill_ratio_to_sas_metadata[layer_name] = _npu_sparse_attn_sharedkv_metadata_maybe_mock(
    ...
    def build_decode_metadata(
        ...
        if self.compressor_ratio <= 1:
            if self.decode_ratio_to_sas_metadata.get(layer_name) is None:
                self.decode_ratio_to_sas_metadata[layer_name] = _npu_sparse_attn_sharedkv_metadata_maybe_mock(
                    num_heads_q=n_local_heads,
                    num_heads_kv=1,
                    head_dim=self.model_config.get_head_size(),
    ...
    def build_prefill_metadata_for_drafting(
        ...
        **kwargs,
        ...
        sas_metadata = _npu_sparse_attn_sharedkv_metadata_maybe_mock(
            num_heads_q=n_local_heads,
            num_heads_kv=1,
            head_dim=self.model_config.get_head_size(),
            cu_seqlens_q=prefill_query_start_loc,
    ...
    def build_decode_metadata_for_drafting(
        ...
        **kwargs,
        ...
        decode_sas_metadata = _npu_sparse_attn_sharedkv_metadata_maybe_mock(
            num_heads_q=n_local_heads,
            num_heads_kv=1,
            head_dim=self.model_config.get_head_size(),
            cu_seqlens_q=query_start_loc,
