...
async def _forward_tokenize(request: Request):
    req_data = await request.json()
    headers = {"Authorization": f"Bearer {os.environ.get('OPENAI_API_KEY')}"}
    servers = proxy_state.prefillers or proxy_state.decoders
    if not servers:
        raise RuntimeError("No backend servers available")

    server = servers[0]
    response = await server.client.post("/tokenize", json=req_data, headers=headers)
    if response.status_code == 404:
        response = await server.client.post(f"{server.root_url}/tokenize", json=req_data, headers=headers)
...
async def handle_tokenize(request: Request):
    return await _forward_tokenize(request)
